...
genomes (hg19, mm9 or mm10), use the \Rcode{loadHelperFile()} function:

<<getPathHelperFiles, print=FALSE, eval=TRUE, echo=FALSE, results=hide>>=
@
...
mappability files for the reference genomes hg19, mm9 and mm10. In addition,
it contains a blacklist filter to remove regions that display CNV. Files in the
returned folder are stored as GRanges objects from the \Biocpkg{GenomicRanges} 
package.
